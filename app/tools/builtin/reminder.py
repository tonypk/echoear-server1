...
@register_tool(
    "reminder.set",
    description="Set a reminder for a specific date/time, optionally recurring",
    params=[
        ...
        ToolParam("recurrence", description="recurrence rule: 'daily', 'weekly', 'monthly', 'weekdays', or 'HH:MM'", required=False),
    ],
    category="reminder",
)
async def reminder_set(datetime_iso: str, message: str, response: str = "", recurrence: str = "", session=None, **kwargs) -> ToolResult:
    try:
    ...
    is_recurring = bool(recurrence and recurrence.strip())
    if is_recurring:
        from ...recurrence import calculate_next_occurrence
        # Test if the rule is valid by calculating next occurrence
        if not calculate_next_occurrence(remind_at, recurrence):
            return ToolResult(type="tts", text="抱歉，我不理解这个循环规则，请换个说法。")
    ...
    try:
        ...
        async with async_session_factory() as db:
            reminder = Reminder(
                ...
                recurrence_rule=recurrence.strip() if is_recurring else "",
            )
            db.add(reminder)
            await db.commit()
            logger.info(f"Reminder saved: '{message}' at {remind_at}, recurring={is_recurring}, rule={recurrence}")
    except Exception as e:
        logger.error(f"Failed to save reminder: {e}")
        return ToolResult(type="error", text="保存提醒失败，请稍后再试。")

    if is_recurring:
        return ToolResult(type="tts", text=response or f"好的，已设置循环提醒：{message}")
